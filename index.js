...
const allSockets = new Set();  // every live connection, for the heartbeat sweep
...
function sendPing(sock) { try { sock.write(Buffer.from([0x89, 0x00])); } catch (_) {} }
...
function handleFrame(sock, opcode, payload) {
  sock.alive = true; // any inbound frame proves the peer is there
...
function registerUser(sock, user) {
  ...
  for (const s of [...set]) if (!s.wsOpen || s.destroyed) set.delete(s);
  set.add(sock);
  adoptRooms(sock);
}
// A reconnecting player must take over the seat held by their previous socket,
// otherwise the match stays bound to a dead connection: no moves, no result.
function adoptRooms(sock) {
  if (!sock.user) return;
  for (const [, players] of rooms) {
    for (const other of [...players]) {
      if (other !== sock && other.user && other.user.id === sock.user.id) {
        players.delete(other);
        players.add(sock);
      }
    }
  }
...
// Insertion order: oldest -> newest. The newest socket is the live one after a reconnect.
...
function attachAndNotify(t, m, room, self, color, opp) {
  ...
  const primary = socks[socks.length - 1];
...
server.on('upgrade', (req, socket) => {
  ...
  const drop = () => { socket.wsOpen = false; allSockets.delete(socket); onDisconnect(socket); };
  socket.on('close', drop);
  socket.on('error', drop);
});

// Heartbeat: a half-open TCP connection (mobile network drop, killed client) never
// fires 'close', so without this a dead socket keeps holding tournament seats and
// casual rooms. One missed pong cycle and we tear it down ourselves.
const HEARTBEAT_MS = Number(process.env.HEARTBEAT_MS || 25000);
setInterval(() => {
  for (const sock of [...allSockets]) {
    if (!sock.alive) { sock.wsOpen = false; sock.destroy(); allSockets.delete(sock); onDisconnect(sock); continue; }
    sock.alive = false;
    sendPing(sock);
  }
}, HEARTBEAT_MS).unref();
